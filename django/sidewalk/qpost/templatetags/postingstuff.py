...
@register.filter
def getqcertificate(quest, username):
	photos = []
	for challenge in quest.challenge_set.all():
		for qphoto in challenge.challengecertificate_set.all():
			if qphoto.user_submitted_name == username:
				photos.append(qphoto.ver_photo)
	if len(photos) > 0:
...
def getqverificationurl(quest, username):
	urls = []
	for challenge in quest.challenge_set.all():
		for qphoto in challenge.questverificationphoto_set.all():
			if qphoto.user_submitted_name == username:
				urls.append(qphoto.photo.url)
	return choice(urls)
